#!/usr/bin/env python3
"""
OPTIMIZED cleanup script to re-run HubSpot enrichment for Jobs 23 and 25.
- Skips domain normalization for companies that already have domains
- Uses maximum batch size of 100 for HubSpot API
Run this script in the Render shell: python cleanup_hubspot_enrichment_optimized.py
"""
...
def main():
    ...
    try:
        # OPTIMIZED: Only normalize domains for companies MISSING domain
        ...
        for job_id in target_job_ids:
            ...
            if companies_missing_domain:
                ...
                session.commit()
                logger.info(f"Job {job_id}: Domain normalization complete for {len(companies_missing_domain)} companies.")
            else:
                logger.info(f"Job {job_id}: No companies missing domain, skipping normalization.")
        ...
        for job_id in target_job_ids:
            ...
            for offset in range(0, total_count, chunk_size):
                ...
                # OPTIMIZED: Process in batches of 100 (HubSpot API max)
                batch_size = 100
        ...
        logger.info("\n🚀 OPTIMIZED cleanup script completed successfully!")
